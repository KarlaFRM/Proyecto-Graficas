...
function plane( width, height ) {
	return function( unit, vect ) {
		var x = ( unit - 0.5 ) * width;
		var y = ( vect + 0.5 ) * height;
		var z = 0;
...
function Cloth( width, heigth ) {
	...
	for ( vect = 0; vect <= heigth; vect ++ ) {
		for ( unit = 0; unit <= width; unit ++ ) {
			particles.push(
				//X,Y,Z
				new Particle( unit / width, vect / heigth, vect / heigth, 1.2 )
			);
...
function movements( time ) {
	...
	sphereCoor.z = 0;
	sphereCoor.x = 0;
	sphereCoor.y = -40;//radio de la esfera - 2
...
//Se manda acceleration llamar main y la animacion
main();
animate();
